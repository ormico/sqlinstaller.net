    ...
    public class ZipEntry
    {
        ...
        internal static ZipEntry Create(String filename, string DirectoryPathInArchive)
        {
            ...
            if (entry._LastModified.IsDaylightSavingTime())
            {
                ...
                entry._LastModDateTime = SQLInstaller.Core.Zip.Shared.DateTimeToPacked(AdjustedTime);
            }
            else
                entry._LastModDateTime = SQLInstaller.Core.Zip.Shared.DateTimeToPacked(entry._LastModified);
